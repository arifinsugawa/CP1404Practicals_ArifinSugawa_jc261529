...
###
 ...
 |  """
    ...
    def get_ascii_leaves(self):
        ...
        return result + super().get_ascii_leaves()
   ...
   |    """
    ...
    def grow(self, sunlight, water): 
        ...
        if (random.randint(0,sunlight) > 2):
            self._levels += 1
            self._leaves = 1 + (self._levels - 1) * 2

    def get_ascii_leaves(self):
        result = ''  
        x = 1
        num_space = 1 + (self._levels - 1) * 2 
        for x in range(1,self._levels):
            result += " " * int(num_space / 2)
            result += '*' * (1 + (x - 1) * 2)
            result += " " * int(num_space / 2) + "\n"
            num_space -= 2
